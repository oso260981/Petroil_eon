view: vis_ventas {
  derived_table: {
    sql: SELECT *,ROW_NUMBER() OVER(
       ORDER BY nb_Cliente) AS RowNum FROM `eon-internal-bigquery.POC.petroil2` where nb_TipoFilial="NO Filial venta" and nb_cliente !="CLIENTES PUBLICO EN GENERAL " and nb_FamiliaProducto in ("Asfaltos","Diesel","Combustoleos","Lubricantes","IFO","Gasolinas")
      ;;
  }
